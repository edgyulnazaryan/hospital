...
$(document).ready(function()
{
    ...
    // show_events(events, year, months[date.getMonth()], today);
});
...
function date_click(event) {
    ...
    show_events(event.data.events, event.data.year, event.data.month, event.data.day);
};
...
function new_event(event) {
    ...
    $("#ok-button").unbind().click({date: event.data.date}, function() {
        ...
        if(name.length === 0) {
            ...
            new_event_json(name, year, count, date, day);
            date.setDate(day);
            init_calendar(date);
        }
    });
...
function new_event_json(name, count, year, date, day) {
    var event = {
        "occasion": name,
        "invited_count": count,
...
function show_events(events, year, month, day) {
	
	var date_graph = year + '-' + month + '-' + day
	
	$.ajax({
        url:"{{ route('search_graph', $data[0]['id']) }}",
        method:'get',
        data: {date:date_graph},
        dataType: "text",
        ...
        {	
        	...
        	{
        		$(".event-card").append("<ul class='list-group'>")
        		for (var j = 0; j < data[i]['time'].length; j++) 
        		{
    				$(".event-card").append("<li class='list-group-item list-group-item-action list-group-item-light'>"+data[i]['time'][j]+"<button class='btn btn-danger btn-sm btn-remove' style='margin-left:245px'>X</button></li>");
        		}
        		$(".event-card").append("</ul>")
        	}
        }
    })
